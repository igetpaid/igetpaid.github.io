      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        {albums.length === 0 ? (
          ...
          >
            ...
            <p className="text-slate-500 text-lg">Галерея пуста</p>
            <p className="text-slate-400 text-sm mt-1">
              Добавь изображения в <code className="text-xs bg-slate-100 px-1.5 py-0.5 rounded">public/gallery/</code>
            </p>
          </motion.div>
        ) : (
          <>
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                    activeAlbum === album.id
                      ? 'bg-vk-blue text-white shadow-md shadow-vk-blue/20'
                      : 'bg-white text-slate-600 border border-slate-200 hover:border-vk-blue/30 hover:text-vk-blue'
                  }`}
                >
            ...
            {currentAlbum && (
              <motion.div
                ...
                className="mt-8"
              >
                <p className="text-sm text-slate-500 mb-4">{currentAlbum.description}</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  {currentAlbum.images.map((image, i) => (
                    <button
                      key={i}
                      onClick={() => openLightbox(i)}
                      className="group aspect-square rounded-xl overflow-hidden bg-slate-100 border border-slate-200 hover:border-vk-blue/30 hover:shadow-lg transition-all duration-200"
                    >
