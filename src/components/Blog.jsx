  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-4 sm:px-6">
        ...
        >
          {blogPosts.length === 0 && (
            <div className="text-center py-16 rounded-2xl border border-dashed border-[var(--section-border)]">
              <BookOpen className="w-12 h-12 text-[var(--section-muted)] mx-auto mb-4" />
              <p className="text-[var(--section-text-secondary)] text-lg">Записей пока нет</p>
              <p className="text-[var(--section-text-secondary)] text-sm mt-1">Скоро здесь появятся первые заметки</p>
            </div>
          )}
          ...
          {blogPosts.map((post) => {
            ...
            return (
              <article
                key={post.id}
                className="p-6 rounded-2xl bg-[var(--section-card-bg)] border border-[var(--section-border)] hover:border-amber-300/40 transition-all duration-200"
              >
                {/* Header */}
                <div className="flex items-center gap-3 text-sm text-[var(--section-text-secondary)] mb-3">
                ...
                {/* Title */}
                <h3 className="text-xl font-bold text-[var(--section-text)]">{post.title}</h3>
                <p className="mt-2 text-sm text-[var(--section-text-secondary)] leading-relaxed">{post.excerpt}</p>
                ...
                {isExpanded && (
                  <motion.div
                    ...
                    className="mt-4 pt-4 border-t border-[var(--section-border)]"
                  >
                    <div className="text-sm text-[var(--section-text)] leading-relaxed whitespace-pre-line">
                      {post.content}
                    </div>
                  </motion.div>
                )}
